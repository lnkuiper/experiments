...
string SimulateRowReOrder(const idx_t row_ids[], const idx_t &count, const idx_t &columns, const idx_t &col_width) {
    ...
    return CreateOutput("row", {count, columns, col_width, total_duration, reorder_duration, scatter_duration,
                        gather_duration}, 8);
}
...
void SortColumn(unique_ptr<data_t[]> &row_id_col, vector<unique_ptr<data_t[]>> &columns, const idx_t &count) {
    ...
    if (columns.size() == 0) {
        ...
        sort(row_ids, row_ids + count, [&row_ids, &col] (const ROWID &lhs, const ROWID &rhs) -> bool { 
...
string SimulateRowComparator(const idx_t &count, const idx_t &columns, bool branchless) {
    ...
    return CreateOutput(category, {count, columns, sizeof(ROWID), sizeof(T), total_duration, sort_duration,
                        scatter_duration, gather_duration}, 9);
}
...
void RadixSort(data_ptr_t orig_ptr, const idx_t &count, const idx_t &row_width,

...
for i in range(1, len(columns) + 1):
    with open(f'sql/{query_count}.sql', 'w+') as f:
        print('CREATE TABLE output AS SELECT ' + ', '.join(columns[:i]) + ' FROM catalog_sales ORDER BY cs_item_sk, cs_quantity;', file=f)
    with open(f'clickhouse/{query_count}.sql', 'w+') as f:
        print('CREATE TABLE output ENGINE = Memory AS SELECT ' + ', '.join(columns[:i]) + ' FROM catalog_sales ORDER BY cs_item_sk, cs_quantity;', file=f)
        query_count += 1

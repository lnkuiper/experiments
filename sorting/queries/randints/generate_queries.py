...
with open('clickhouse/100_asc.sql', 'w+') as f:
    print('CREATE TEMPORARY TABLE output ENGINE = File(Native) AS SELECT * from ints100_asc ORDER BY i;', file=f, end='')
...
with open('clickhouse/100_desc.sql', 'w+') as f:
    print('CREATE TEMPORARY TABLE output ENGINE = File(Native) AS SELECT * from ints100_desc ORDER BY i;', file=f, end='')
...
for i in range(1, 11):
    ...
    with open(f'clickhouse/{i * 10}.sql', 'w+') as f:
        print(f'CREATE TABLE output ENGINE = File(Native) AS SELECT * from ints{i * 10} ORDER BY i;', file=f, end='')

#!/bin/bash
. ../../pathvar.sh
docker run -i --rm --link clickhouse_server:clickhouse-server yandex/clickhouse-client --host clickhouse-server --multiquery < $PATHVAR/data/tpcds/schema/clickhouse_schema.sql
